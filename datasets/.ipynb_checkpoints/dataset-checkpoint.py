...
class dataset(Dataset):
    ...
    RSNA dataset
    """
    def __init__(self, ids, images, labels):
        self.ids = ids
    ...
    def getTarget(self, index):
        target = dict()
        for _, row in self.labels[self.labels.patientId == self.ids[index]]:
            x,y,h,w,label = row
            if x != np.nan:
                target["boxes"].append([x,y, x + width, y+height ])
                target["labels"].append(labels)
                target["area"].append(width*height)

        target["image_id"] = torch.tensor([index])
        targte["labels"] = torch.tensor(target.labels)
